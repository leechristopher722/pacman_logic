...
from logic import conjoin, conjuncts, disjoin
...
def sentence3() -> Expr:
    ...
    first = PacmanAlive_1 % (PacmanAlive_0 &
                             ~ PacmanKilled_0 | (~ PacmanAlive_0 & PacmanBorn_0))
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    for A, B in itertools.combinations(literals, 2):
        inverseItem = logic.disjoin(~A, ~B)
        conjuncts.append(inverseItem)

    return logic.conjoin(conjuncts)
    "*** END YOUR CODE HERE ***"
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]] = None) -> Expr:
    ...
    Previous = logic.disjoin(possible_causes)
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for x, y in all_coords:
        PacmanWall = PropSymbolExpr(wall_str, x, y)
        PacmanPos = PropSymbolExpr(pacman_str, x, y, time=t)
        pacphysics_sentences.append(PacmanWall >> ~PacmanPos)
    ...
    pacphysics_sentences.append(exactlyOne(
        [PropSymbolExpr(action, time=t) for action in DIRECTIONS]))
    ...
    # Part 5
    if successorAxioms is not None and walls_grid is not None:
        if t:
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model1 = findModel(logic.conjoin(KB) & at_x1_y1)
    model2 = findModel(logic.conjoin(KB) & ~at_x1_y1)
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        res = findModel(logic.conjoin(KB) & GoalState)

        if res:
            return extractActionSequence(res, actions)

        KB.append(exactlyOne([PropSymbolExpr(action, time=t)
                              for action in actions]))
        KB.append(logic.conjoin([pacmanSuccessorAxiomSingle(
            x, y, t+1, walls_grid) for x, y in non_wall_coords]))
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne([PropSymbolExpr(action, time=t)
                              for action in actions]))
        KB.append(logic.conjoin([pacmanSuccessorAxiomSingle(
            x, y, t+1, walls) for x, y in non_wall_coords]))

        foods = []
        for x, y in non_wall_coords:
            Food_0 = PropSymbolExpr(food_str, x, y, time=t)
            Food_1 = PropSymbolExpr(food_str, x, y, time=t+1)
            Pos = PropSymbolExpr(pacman_str, x, y, time=t)

            Sentence = (Pos | ~Food_0) % ~Food_1
            foods.append(Sentence)
        KB.append(logic.conjoin(foods))

        GoalState = ~ logic.disjoin(
            [PropSymbolExpr(food_str, x, y, time=t) for x, y in non_wall_coords])
        res = findModel(logic.conjoin(KB) & GoalState)
...
def addPacphysics(agent, KB, t, all_coords, non_outer_wall_coords, walls_grid, slam):
    if slam:
...
def findLocations(KB, non_outer_wall_coords, t):
    possible_locations = []
    added = []

    for x, y in non_outer_wall_coords:
        conjoined = logic.conjoin(KB)
        posXY = PropSymbolExpr(pacman_str, x, y, time=t)

        if not entails(conjoined, ~posXY):
            possible_locations.append((x, y))

        if entails(conjoined, posXY):
            added.append(posXY)
        elif entails(conjoined, ~posXY):
            added.append(~posXY)

    return KB + added, possible_locations

def findWallLocations(KB, non_outer_wall_coords, known):
    for x, y in non_outer_wall_coords:
        conjoined = logic.conjoin(KB)
        wallXY = PropSymbolExpr(wall_str, x, y)

        if entails(conjoined, wallXY):
            KB.append(wallXY)
            known[x][y] = 1
        elif entails(conjoined, ~wallXY): 
            KB.append(~wallXY)
            known[x][y] = 0
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = addPacphysics(agent, KB, t, all_coords, non_outer_wall_coords, walls_grid, False)
        KB, possible_locations = findLocations(KB, non_outer_wall_coords, t)
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = addPacphysics(agent, KB, t, all_coords, non_outer_wall_coords, known_map, False)
        KB, known_map = findWallLocations(KB, non_outer_wall_coords, known_map)
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB = addPacphysics(agent, KB, t, all_coords, non_outer_wall_coords, known_map, True)
        KB, known_map = findWallLocations(KB, non_outer_wall_coords, known_map)
        KB, possible_locations = findLocations(KB, non_outer_wall_coords, t)
